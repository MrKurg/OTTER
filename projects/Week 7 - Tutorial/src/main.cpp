...
int main() {
	...
	if (loadScene) {
		...
		Guid daggerMesh = ResourceManager::CreateMesh("Parrying_Dagger.obj");
		...
		Guid bloodTexture  = ResourceManager::CreateTexture("textures/blood.png");
		Guid tableTexture  = ResourceManager::CreateTexture("textures/table.jpg");
		...
		MaterialInfo::Sptr bloodMaterial = std::make_shared<MaterialInfo>();
		bloodMaterial->Shader = scene->BaseShader;
		bloodMaterial->Texture = ResourceManager::GetTexture(bloodTexture);
		bloodMaterial->Shininess = 8.0f;
		scene->Materials[bloodMaterial->GetGUID()] = bloodMaterial;

		MaterialInfo::Sptr tableMaterial = std::make_shared<MaterialInfo>();
		tableMaterial->Shader = scene->BaseShader;
		tableMaterial->Texture = ResourceManager::GetTexture(tableTexture);
		tableMaterial->Shininess = 8.0f;
		scene->Materials[tableMaterial->GetGUID()] = tableMaterial;
		...
		RenderObject blood = RenderObject();
		blood.MeshBuilderParams.push_back(MeshBuilderParam::CreatePlane(ZERO, UNIT_Z, UNIT_X, glm::vec2(0.5f)));
		blood.GenerateMesh();
		blood.Position = glm::vec3(0.0f, 0.0f, 2.0f);
		blood.Name = "Blood";
		blood.Material = bloodMaterial;
		scene->Objects.push_back(blood);
		...
		dagger.Position = glm::vec3(0.0f, 0.0f, 1.0f);
		dagger.Mesh = ResourceManager::GetMesh(daggerMesh);
		dagger.Material = monkeyMaterial;
		dagger.Rotation.z = 180.0f;
		dagger.Name = "Dagger";
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (isRotating) {
			monkey1->Rotation += glm::vec3(0.0f, 0.0f, dt * 720.0f);
