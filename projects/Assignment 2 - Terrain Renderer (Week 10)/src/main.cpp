...
// Using Week 10 Tutorial Starter as the starter of Assignment 2
...
glm::ivec2 windowSize = glm::ivec2(1600, 800);
// The title of our GLFW window
std::string windowTitle = "Kevin Huang - 100788603 and Ewan Chang - 100787343";
...
void CreateScene() {
	...
	if (loadScene) {
		...
		// Shaders
		Shader::Sptr terrainShader = ResourceManager::CreateAsset<Shader>(
			std::unordered_map<ShaderPartType, std::string>{
		...
		Shader::Sptr waterShader = ResourceManager::CreateAsset<Shader>(
			std::unordered_map<ShaderPartType, std::string>{
		...
		// Textures
		Texture2D::Sptr		sandTex = ResourceManager::CreateAsset<Texture2D>("terrain/sand_XL.png");
		Texture2D::Sptr		grassTex = ResourceManager::CreateAsset<Texture2D>("terrain/grass_XL.png");
		Texture2D::Sptr		rockTex = ResourceManager::CreateAsset<Texture2D>("terrain/rock_XL.png");
		Texture2D::Sptr		snowTex = ResourceManager::CreateAsset<Texture2D>("terrain/snow_XL.png");

		Texture2D::Sptr		waterTex = ResourceManager::CreateAsset<Texture2D>("terrain/water.png");
		Texture2D::Sptr		terrainHM = ResourceManager::CreateAsset<Texture2D>("terrain/DarkFog.png");
		...
		Material::Sptr terrainMat = ResourceManager::CreateAsset<Material>(terrainShader);
		{
			...
			terrainMat->Set("heightMap", terrainHM); // Height Map

			terrainMat->Set("u_Material.sand", sandTex); // Sand sampler2D
			terrainMat->Set("u_Material.grass", grassTex); // Grass sampler2D
			terrainMat->Set("u_Material.rock", rockTex); // Rock sampler2D
			terrainMat->Set("u_Material.snow", snowTex); // Snow sampler2D

			terrainMat->Set("u_Material.sandPos", -1.0f); // Sand start pos
			terrainMat->Set("u_Material.grassPos", 3.0f); // Grass start pos
			terrainMat->Set("u_Material.rockPos", 6.0f); // Rock start pos
			terrainMat->Set("u_Material.snowPos", 9.0f); // Snow start pos
		}
		...
		Material::Sptr waterMat = ResourceManager::CreateAsset<Material>(waterShader);
		{
			...
			waterMat->Set("u_VertexMaterial.waterHeight", waterTex); // Water Height Texture
			waterMat->Set("u_VertexMaterial.timeMultiplier", 3.0f); // Time X
			waterMat->Set("u_VertexMaterial.sinMultiplier", 0.17f); // Sin X
			waterMat->Set("u_VertexMaterial.zPosMultiplier", 10.0f); // Vertex Height X

			waterMat->Set("waterTexture", waterTex); // Water Texture
			waterMat->Set("waterMoveUV", glm::vec2(0.0f, 0.01f)); // Vertex Height X
		}

		// Objects 
		GameObject::Sptr camera = scene->CreateGameObject("Main Camera");
		{
			camera->SetPostion(glm::vec3(25.0f, 25.0f, 30.0f));
			camera->LookAt(glm::vec3(0.0f));
			camera->SetRotation(glm::vec3(39.0f, 0.0f, 135.0f));
		...
		// Terrain
		GameObject::Sptr terrainObj = scene->CreateGameObject("Terrain");
		{
			terrainObj->SetPostion(glm::vec3(0.0f, 0.0f, 0.0f));
			terrainObj->SetScale(glm::vec3(3.0f, 3.0f, 1.0f));

			RenderComponent::Sptr renderer = terrainObj->Add<RenderComponent>();
			renderer->SetMesh(planeMesh);
			renderer->SetMaterial(terrainMat);
		}

		// Water
		GameObject::Sptr waterObj = scene->CreateGameObject("Water");
		{
			waterObj->SetPostion(glm::vec3(0.0f, 0.0f, 1.5f));
			waterObj->SetScale(glm::vec3(3.0f, 3.0f, 1.0f));

			RenderComponent::Sptr renderer = waterObj->Add<RenderComponent>();
			renderer->SetMesh(planeMesh);
